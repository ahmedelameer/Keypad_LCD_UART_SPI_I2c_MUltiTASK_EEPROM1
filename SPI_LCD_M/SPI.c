...
void SPI_sendByte(const unit8 data)
{
	...
	while(Pin_is_clear(SPSR,SPIF)){} //wait until SPI interrupt flag=1 (data is sent correctly)
}

unit8 SPI_recieveByte(void)
{
   while(Pin_is_clear(SPSR,SPIF)){} //wait until SPI interrupt flag=1(data is receive correctly)
